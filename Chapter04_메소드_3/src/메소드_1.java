/*  
 *  1. 형식 / 문법
 *  2. 간단한 예제 => 흐름 (동작 => 호출)
 *  3. 약간 응용 => 확장
 *  4. 기간이 지나면 감 => 2~3월
 *  
 *  ---------------------------------
 *  데이터 저장
 *  	= 변수 : 한개 데이터 저장
 *  	= 배열 : 여러개를 연속적으로 저장 => 인덱스 : 고정적 / 같은 데이터형
 *  	= 클래스 : 여러개 데이터 저장 => 다른 데이터형 => 동적 (new)
 *  	----------------------- 메모리 저장
 *  	= 파일 => 종속(파일과 파일의 연결이 어렵다)
 *  	= RDBMS(오라클)
 *  사용자가 요청하면 => 처리
 *  	= 연산자
 *  	= 제어문
 *  	-------- 관련된 명령문 (기능을 모아서 처리) => 메소드
 *  사용자가 요청한 위치에 처리 결과 출력
 *  	= 브라우저 출력 (웹)
 *  
 *  -----------------------------------------------
 *  일반자바 => 오라클 연결 자바 ==> 웹 연결 자바
 *  -----------------------------------------------
 *  | 통합 : 스프링
 *  
 *  메소드 : => 결과값(리턴형), 사용자 요청값 (매개변수)
 *  		  -----------  ------------------
 *  		   |1개만 가능					|제한이 없다
 *  		   |=> 자바/C => 1개만 사용		|=> 매개변수를 많이 사용하면
 *  		   |return 값				|	=> 묶어서 설정
 *  		   |  여러개 값이 있는 경우 : 배열/클래스
 *  		   |=> 파이썬 => 여러개 가능
 *  		   |return 값, 값, 값...
 *  		   |변수, 변수, 변수 = 메소드
 *  
 *  		=> 프로그램
 *  		   기본
 *  			 = 입력 기능
 *  			 = 요청처리 기능 => 세분화
 *  			 = 출력기능
 *  			 = 모든 메소드는 한개의 기능만 수행 => 유지보수
 *  				=> 결제창 => 등록
 *  			 = 교통시스템
 *  				사고 / 공사 => 출력
 *  				CCTV => 위도 / 경도 / opencv
 *  		=> 프로그램 => 개발보다 유지보수에 초점
 *  						   ------- 가독성, 재사용
 *  					 메소드 변경 / 메소드 추가
 *  	형식)
 *  		[접근지정어][옵션] 리턴형 메소드명(매개변수) => 선언부
 *  
 *  	접근지정어
 *  		default
 *  		=> 같은 폴더에 존재
 *  		   ------- 패키지
 *  	옵션
 *  		final : 변경이 없다
 *  		abstract : 선언만
 *  		static : 자동 저장 => 공유
 *  	리턴형 : 처리 결과값 / 한개만 설정이 가능
 *  		기본형(int, String, double....)
 *  		배열
 *  		클래스
 *  	메소드명 : 기능별 구분자
 *  			=> 메소드명이 동일하면 안된다
 *  			   -------------------
 *  			   기능이 같고 처리가 다른경우 같은 메소드명을 사용 => 오버로딩
 *  			=> 문법은 변수 식별자와 동일
 *  	매개변수 : 사용자 요청값
 *  	
 *  	메소드 경우의 수
 *  	----------------------------
 *  		리턴형		매개변수
 *  		  O			  O
 *  		String = substring(int begin)
 *  		------			   ---------
 *  		String = substring(int begin, int end)
 *  	----------------------------
 *  		  O			  X
 *  		String trim() => 좌우 공백만 제거
 *  		double random(); => 0.0 ~ 0.99
 *  	----------------------------
 *  		  X			  O
 *   (void)	System.out.println(String s);
 *   ------ 결과값이 없는 경우 => 메소드 자체 처리
 *   	|=> 오라클 연결시에 insert/update/delete
 *  	----------------------------
 *  		  X			  X
 *   (void)	System.out.println(); => 줄바꿈 / HTML => <br>
 *  	----------------------------
 *  
 *  메소드 호출
 *  --------
 *  	메소드명(전송하는 값) => void
 *  	데이터형 변수 = 메소드명(전송하는 값)
 *  			|		|
 *  			---------
 *  	=> 왼쪽에 있는 데이터형이 크거나 같다 => 권장(같은 데이터형)
 *  	=> 통계 / 회계
 *  
 *  메소드 제작
 *  1) 반복이 많은 경우
 *  2) 기능별로 나눠서 처리 => 단락
 *  3) 다른 클래스와 연결
 *  -------------------------
 *  메소드 사용 목적
 *  1) 반복제거
 *  2) 코드 관리가 쉽게 (소스를 간결)
 *  3) 재사용
 *  
 *  메소드 호출 => 실행 순서
 *  
 *  void display()
 *  {
 *  	1문장 --- 2
 *  	2문장 --- 3
 *  	3문장 --- 4
 *  	4문장 --- 5
 *  	5문장 --- 6
 *  }
 *  // 메소드 => 시작 (JVM=>가장 먼저 호출)
 *  main()
 *  {
 *  	1문장 --- 1
 *  	display(); ==> 복귀
 *  	2문장 --- 7
 *  	3문장 --- 8
 *  	display(); //호출한 메소드를 처음부터 마지막까지 수행후 복귀
 *  }
 *  
 */		
public class 메소드_1 {
	static void display() {
		int i = 1; //display{}안에서만 사용이 가능
		System.out.println("i = " + i);
		i++;
		//{}이 종료되면 자동으로 메모리에서 사라진다 => 지역변수
	}
	public static void main(String[] args) {
		System.out.println("main start");
		display();
		display();
		display();
		System.out.println("main end");
	}
	
}
