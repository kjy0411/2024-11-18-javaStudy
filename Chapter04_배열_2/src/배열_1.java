 ...
 *  ----------------------------------- 자바의 기본 형식
 *  => 객체 지향 프로그램 (5장)
 *  배열 : 관련 데이터를 여러개 모아서 한개의 이름으로 관리
 *  ---
 *    1) 장점 => 여러개 변수를 한번에 처리가 가능
 *    			------------------
 *    			연속적인 메로리 구조 => 메모리 크기가 동일
 *    			=> 같은 데이터형만 모아서 관리
 *    2) 단점 => 가변형 (컬렉션)
 *  	 => 한번 선언하면 추가/삭제가 어렵다
 *  				  ------- 새롭게 큰/작은 배열을 생성
 ...
 *     			------ 모든 클래스는 값을 지정해주지 않으면 기본적으로 null
 *     	*** 배열
 *     	int[] 배열명
 *     	double[] 배열명
 *     	String[] 배열명
 ...
 *     	boolean[] 배열명
 *      
 *      => 선언시에 저장 갯수 확인 => 개수가 고정된다 => 저장된ㄱ
 *      int arr = {1,2,3,4,5} => 5개 저장
 *      int arr = new int[5] => 5개 저장
 *      	=> 개수가 고정된다
 *      		- 저장된 값은 변경이 가능
 *      		- 메모리 크기 변경 불가능
 *      			- arraycopy/새로운 배열
 *      		- 얕은 복사 => 같은 주소를 이용하는 배열 생성
 *      		- 깊은 복사 => 새로운 배열 생성 => clone()
 *      			
 *     2) 선언 후에 초기화
 *     	int[] arr;
 *     	=> arr = new int[]{1,2,3,4,5}; => 메소드 이용할때 사용
 *     
 *     3) 출력 (데이터 읽기)
 *     	int[] arr = {1,2,3,4}
 *     	= 일반 for문 => 인덱스 이용
 ...
 *      	}
 *      = 향상된 for문(for-each문) 이용 => 실제 배열에 저장된 데이터 읽기
 *      	브라우저에 출력시 주로 사용
 *      	=> 변경 불가능/출력만 가능
 *      	for(int i : arr){
 *      		System.out.println(i);
 *      	}
 *      = 라이브러리(Arrys.toString()) 이용
 *      	System.out.println(Arrys.toString(배열명));
 *      4) 메모리에 저장되는 과정 (메모리 주소를 참조해서 데이터 관리 => 참조변수)
 *         int[] arr = new int[5]
 *         ---------   ----------
 *         Stack(데이터가 있는 주소)			Heap(실제 데이터가 저장된 메모리)
 *         
 *         ----arr-----				 ---------------------------------------
 *         	   null => 주소가 배정이 안됨	0	|	0	|	0	|	0	|	0
 *         ------------				 |--------------------------------------
 *         							0x100 => 가장
