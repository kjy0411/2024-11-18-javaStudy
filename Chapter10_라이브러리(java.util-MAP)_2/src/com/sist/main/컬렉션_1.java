 ...
 *  1) 다수의 데이터를 쉽고 효율적으로 처리가 가능하게 만든 표준화된 클래스 집합 (자바에서 지원하는 라이브러리)
 *     ----------							   ----
 *  2) 자료구조 => CRUD (읽기 / 추가 / 삭제 / 수정)
 *  3) 배열의 단점 보완 => 고정 => 가변 => 동적으로 변환 (메모리 추가, 삭제 자동 조절)
 ...
 *  	|				|				|
 *    List			   Set			   Map => 인터페이스
 *  순서가 있다(인덱스)	순서가 없다		key,value를 동시에 저장
 *  데이터 중복 허용	데이터 중복허용(X)	key는 중복이 없다
 *  	|				|			value는 중복허용
 *  	|				|
 *  구현된 클래스		
 *  => ArrayList***
 ...
 *     get() : 읽기
 *     remove() : 삭제
 *     ---------------- CRUD (Creat / Read / Update / Delete) : 게시판
 *     						   추가	   읽기    수정       삭제
...
public class 컬렉션_1 {
	...
	public static void main(String[] args) {
		 ...
		 *  LinkedList<String> list = new Linked List<String>();
